...
public class LogicBuilder : MonoBehaviour
{
    ...
    private enum EstadoConstruccion
    {
        ...
        EsperandoDirRadar,      // Dirección del Radar
        EsperandoOperadorIf,
        EsperandoValorIf,       // Valor genérico (Salud)
    ...
    {
        // Cierre automático de número si venimos de escribir Salud
    ...
    {
        // Solo pide dirección, ya no pide rango
    ...
    void OnClick_Numero(int num)
    {
        ...
        {
            // RESTRICCIÓN: Solo aceptamos 0
            if (num == 0)
            {
                ifValor = 0;
                lineaEnConstruccion += "0): "; // Cerramos paréntesis y preparamos acción
                // Truco: No cambiamos de estado "oficialmente" en el enum, 
                // pero ActualizarBotonesUI detectará que acabamos de cerrar el IF
                // y habilitará los botones de acción.
            }
        }
    ...
    void OnClick_Direccion(Direccion dir)
    {
        ...
        switch (estadoActual)
        {
            ...
            case EstadoConstruccion.EsperandoDirRadar:
                // Radar Simplificado: Dirección -> Operador
                ifCondicion = ObtenerTipoRadar(dir);
                lineaEnConstruccion += dirStr + ")"; // Cerramos paréntesis aquí
                estadoActual = EstadoConstruccion.EsperandoOperadorIf;
                break;
        }
        RefrescarTextoScript();
    ...
    void ActualizarBotonesUI()
    {
        ...
        switch (estadoActual)
        {
            ...
            case EstadoConstruccion.EsperandoValorIf:
                // Lógica libre para Salud (acepta menos y numeros)
            ...
            case EstadoConstruccion.EsperandoValorRadar:
                // RESTRICCIÓN ESTRICTA PARA RADAR: Solo el 0
                botonesNumeros[0].interactable = true; // Solo el botón "0"

                // Si ya se presionó el 0 (la linea termina en "0): "), habilitamos acciones
                if (lineaEnConstruccion.EndsWith("0): "))
                {
                    SetGrupoBotones(true, btnMover, btnTorpedo, btnPlantarMina, btnCanon, btnReparar);
                    SetGrupoBotones(false, botonesNumeros); // Bloqueamos números para que no ponga "00"
                }
                break;
        }
    }
